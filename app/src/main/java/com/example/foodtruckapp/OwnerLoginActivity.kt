...
class OwnerLoginActivity : AppCompatActivity() {
    ...
    fun logInOwner() {
        ...
        val loggedInOwner = database.ownerDao().getByEmailAndPassword(email, password)
        if (loggedInOwner == null) {
            showToast("Invalid password")
            return
        }

        CurrentLogin.loginOwner(loggedInOwner)
        showToast("Welcome back ${loggedInOwner.name}")
